...
async function callSnowflakeTool<T = unknown>(
  ...
  try {
    const response = await mcpClient.callTool<T>({ name: toolName, arguments: normalizedArgs });
...
export async function fetchDashboardData(): Promise<DashboardData> {
  ...
  try {
    ...
    if (databases.status === 'fulfilled' && databases.value.length > 0) {
      ...
      const schemas = await listSchemas(defaultDatabase).catch(() => []);
      data.schemas = schemas;
      data.totalSchemas = schemas.length;

      if (schemas.length > 0) {
        const tables = await listTables(defaultDatabase, schemas[0]).catch(() => []);
        data.tables = tables;
...
export async function fetchQueryHistory(limit = 20): Promise<QueryHistoryEntry[]> {
  try {
    const result = await callSnowflakeTool<SnowflakeQueryResult>('get_query_history', { limit: String(limit) });
    if (!result.rows) return [];
