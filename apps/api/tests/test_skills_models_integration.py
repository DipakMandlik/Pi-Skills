...
from backend.routers import models as models_router_module
from backend.routers.models import router as models_router
from backend.routers.skills import router as skills_router
from backend.schemas.api import ModelConnectivityValidationResponse
...
def _build_app(tmp_path: Path) -> FastAPI:
    ...
    settings = Settings(
        ...
        mcp_auth_fallback_enabled=False,
    )
...
def test_model_configuration_crud_and_validate_flow(tmp_path: Path) -> None:
    ...
    with patch.object(
        ...
        assert update_config_response.json()["base_url"] == "https://api.openai.com/v1/chat/completions"
