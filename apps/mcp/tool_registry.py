...
class ToolRegistry:
    ...
    def _register_tools(self) -> None:
        ...
        self._register(
            ToolDefinition(
                name="create_cortex_agent",
                description="Create a Cortex Agent configuration.",
                input_schema={"type": "object", "required": ["name"], "properties": {"name": {"type": "string"}, "description": {"type": "string"}, "tools": {"type": "array"}}},
                output_schema={"type": "object", "properties": {"agent_name": {"type": "string"}, "status": {"type": "string"}}},
            ),
            self._create_cortex_agent,
        )

        self._register(
            ToolDefinition(
                name="call_cortex_agent",
                description="Send a message to a Cortex Agent.",
                input_schema={"type": "object", "required": ["agent_name", "message"], "properties": {"agent_name": {"type": "string"}, "message": {"type": "string"}}},
                output_schema={"type": "object", "properties": {"agent": {"type": "string"}, "response": {"type": "string"}}},
            ),
            self._call_cortex_agent,
        )
    ...
    def _search_objects(self, args: dict[str, Any]) -> dict[str, Any]:
        ...
        type_filter = ""
        if object_types:
            type_list = ", ".join(f"'{t.upper()}'" for t in object_types)
            type_filter = f"AND TABLE_TYPE IN ({type_list})"

        for db in ([database] if database else self._get_databases()):
            try:
                q = f"""
                    SELECT TABLE_CATALOG, TABLE_SCHEMA, TABLE_NAME, TABLE_TYPE
                    FROM {db}.INFORMATION_SCHEMA.TABLES
                    WHERE UPPER(TABLE_NAME) LIKE '%{keyword_upper}%'
                    {type_filter}
                    ORDER BY TABLE_NAME
                    LIMIT 50
                """.strip()
                res = self.sf.execute_query(q)
                for row in res["rows"]:
                    results.append({
                        "database": row[0],
                        "schema": row[1],
        ...
        if direction in ("upstream", "both"):
            up_query = f"""
                ...
                WHERE OBJECT_NAME = '{table}'
                  AND OBJECT_DATABASE = '{database}'
                  AND OBJECT_SCHEMA = '{schema}'
            """.strip()
            up_result = self.sf.execute_query(up_query)
        ...
        if direction in ("downstream", "both"):
            down_query = f"""
                ...
                WHERE REFERENCED_OBJECT_NAME = '{table}'
                  AND REFERENCED_OBJECT_DATABASE = '{database}'
                  AND REFERENCED_OBJECT_SCHEMA = '{schema}'
            """.strip()
            down_result = self.sf.execute_query(down_query)
    ...
    def _get_access_history(self, args: dict[str, Any]) -> dict[str, Any]:
        ...
        schema = str(args.get("schema", "")).upper()
        ...
        query = f"""
            ...
            WHERE QUERY_START_TIME >= '{start_ts.strftime('%Y-%m-%d %H:%M:%S')}'
              AND ARRAY_CONTAINS('{database}.{schema}.{table}'::VARIANT, DIRECT_OBJECTS_ACCESSED)
        ...
        return {"object": f"{database}.{schema}.{table}", "accesses": accesses, "days": days}
    ...
    def _classify_columns(self, args: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            classify_result = self.sf.execute_query(f"CALL SYSTEM$CLASSIFY('{full_table}', {{'auto_classify': true}})")
    ...
    def _get_credit_usage(self, args: dict[str, Any]) -> dict[str, Any]:
        ...
        query = f"""
            ...
            WHERE START_TIME >= '{start_ts.strftime('%Y-%m-%d %H:%M:%S')}'
    ...
    def _cortex_summarize(self, args: dict[str, Any]) -> dict[str, Any]:
        ...
        query = f"SELECT SNOWFLAKE.CORTEX.SUMMARIZE('{safe_text}') AS summary"
    ...
    def _cortex_sentiment(self, args: dict[str, Any]) -> dict[str, Any]:
        ...
        safe_text = text.replace("'", "''")
        query = f"SELECT SNOWFLAKE.CORTEX.SENTIMENT('{safe_text}') AS sentiment"
    ...
    def _cortex_classify_text(self, args: dict[str, Any]) -> dict[str, Any]:
        ...
        safe_text = text.replace("'", "''")
        cats_json = str(categories).replace("'", "''")
        query = f"SELECT SNOWFLAKE.CORTEX.CLASSIFY_TEXT('{safe_text}', {cats_json}) AS classification"
    ...
    def _cortex_translate(self, args: dict[str, Any]) -> dict[str, Any]:
        ...
        safe_text = text.replace("'", "''")
        query = f"SELECT SNOWFLAKE.CORTEX.TRANSLATE('{safe_text}', '', '{target_lang}') AS translated"
    ...
    def _cortex_analyst_query(self, args: dict[str, Any]) -> dict[str, Any]:
        ...
        query = f"""
            SELECT SNOWFLAKE.CORTEX.ANALYST('{safe_question}', '{model_path}') AS result
        """.strip()
...
"""
    ...
    def _create_cortex_agent(self, args: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            "tools": tools,
            "status": "created",
            "message": f"Agent '{name}' configuration saved. Deploy via Snowflake UI.",
        }

    def _call_cortex_agent(self, args: dict[str, Any]) -> dict[str, Any]:
        agent_name = str(args.get("agent_name", "")).strip()
        message = str(args.get("message", "")).strip()

        if not agent_name or not message:
            raise ValidationError("agent_name and message are required")

        safe_message = message.replace("'", "''")
        query = f"""
            SELECT SNOWFLAKE.CORTEX.AGENT('{agent_name}', '{safe_message}') AS response
        """

        try:
            result = self.sf.execute_query(query)
            response = result["rows"][0][0] if result["rows"] else ""
            return {"agent": agent_name, "message": message, "response": str(response)}
        except Exception as e:
