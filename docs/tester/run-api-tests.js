...
async function testAdminAndAIIntelligence() {
  ...
  const aiAdminEndpoints = [
    { id: 'AI-001', path: '/ai-intelligence/safety/policy' },
    { id: 'AI-002', path: '/ai-intelligence/cache/stats' },
    { id: 'AI-003', path: '/ai-intelligence/anomalies/summary' },
    { id: 'AI-004', path: '/ai-intelligence/scaling/recommendations' },
  ];

  for (const endpoint of aiAdminEndpoints) {
    const adminRes = await req('GET', `${BASE_URL}${endpoint.path}`, null, authHeader('admin'));
    check(adminRes.status === 200, endpoint.id, 'AI-Intelligence', adminRes.status === 200, '200', `${adminRes.status}`);

    const userRes = await req('GET', `${BASE_URL}${endpoint.path}`, null, authHeader('user'));
    check(userRes.status === 403, `${endpoint.id}-RBAC`, 'AI-Intelligence', userRes.status === 403, '403', `${userRes.status}`);
  }
  ...
  // AI intelligence POST functionality checks
  const sanitizePrompt = await req(
    'POST',
    `${BASE_URL}/ai-intelligence/safety/sanitize`,
    { prompt: 'Summarize this compliance incident in 5 bullets.' },
    authHeader('admin')
  );
  const sanitizeOk = sanitizePrompt.status === 200 && typeof sanitizePrompt.data.sanitized === 'string';
  check(sanitizeOk, 'AI-POST-001', 'AI-Intelligence', sanitizeOk, '200 with sanitized:string', `${sanitizePrompt.status}`);

  const moderatePrompt = await req(
    'POST',
    `${BASE_URL}/ai-intelligence/safety/moderate-prompt`,
    { prompt: 'Hello, this is a normal harmless message.', strict_mode: false, user_id: targetUserId },
    authHeader('admin')
  );
  const moderateOk = moderatePrompt.status === 200 && typeof moderatePrompt.data.safe === 'boolean';
  check(moderateOk, 'AI-POST-002', 'AI-Intelligence', moderateOk, '200 with safe:boolean', `${moderatePrompt.status}`);

  const analyzeTask = await req(
    'POST',
    `${BASE_URL}/ai-intelligence/smart-route/analyze`,
    { prompt: 'Create a brief architecture summary for the auth subsystem.' },
    authHeader('admin')
  );
  check(analyzeTask.status === 200, 'AI-POST-003', 'AI-Intelligence', analyzeTask.status === 200, '200', `${analyzeTask.status}`);

  const invalidAnomalyQuery = await req('GET', `${BASE_URL}/ai-intelligence/anomalies/usage?hours=0`, null, authHeader('admin'));
  check(invalidAnomalyQuery.status === 422, 'AI-GET-NEG-001', 'AI-Intelligence', invalidAnomalyQuery.status === 422, '422', `${invalidAnomalyQuery.status}`);

  const aiNoAuth = await req('GET', `${BASE_URL}/ai-intelligence/cache/stats`);
  check(aiNoAuth.status === 401, 'AI-GET-NEG-002', 'AI-Intelligence', aiNoAuth.status === 401, '401', `${aiNoAuth.status}`);

  const optimizePromptUser = await req(
    'POST',
    `${BASE_URL}/ai-intelligence/optimize/prompt`,
    { prompt: 'User should not access admin AI optimization.' },
    authHeader('user')
  );
  check(optimizePromptUser.status === 403, 'AI-POST-004-RBAC', 'AI-Intelligence', optimizePromptUser.status === 403, '403', `${optimizePromptUser.status}`);
}
