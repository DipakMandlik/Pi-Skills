...
def _to_platform_role(role: str) -> str:
    ...
    # Check if Snowflake role name contains admin keywords
...
def _public_user(user: dict[str, Any]) -> dict[str, Any]:
    # Internal session metadata keys are prefixed with underscore.
...
def auth_login(request: AuthLoginRequest) -> dict[str, Any]:
    ...
    try:
        ...
        # Prefer Snowflake-evaluated role when present; fall back to requested role.
        ...
        logger.info("auth_login_success user=%s account=%s role=%s", snowflake_user, request.account, snowflake_role)
        ...
        logger.warning("auth_login_failed user=%s account=%s error=%s", request.username, request.account, sanitize_error(exc))
        raise HTTPException(status_code=401, detail="Snowflake authentication failed. Check your account, username, password, and role.")
...
def call_tool(request: ToolCallRequest, authorization: str | None = Header(default=None)) -> dict[str, Any]:
    try:
        ...
        try:
            result = registry.run_tool(request.name, request.arguments, execution_context=execution_context)
        except TypeError:
            # Backward compatibility with test doubles that still expose the older signature.
