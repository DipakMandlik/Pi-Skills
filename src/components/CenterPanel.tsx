...
export function CenterPanel() {
  ...
  return (
    <div className="flex-1 flex flex-col h-full relative bg-bg-base">
      ...
      <div className="h-16 border-b border-border flex items-center justify-between px-6 relative z-10 bg-panel/80 backdrop-blur-md shadow-sm shrink-0">
        <h2 className="font-display font-medium text-text-main">Workspace</h2>
        <div className="flex items-center gap-3">
          ...
          <button 
            ...
            className="flex items-center gap-2 px-3 py-1.5 bg-white border border-border rounded-lg text-sm font-medium text-text-main hover:bg-slate-50 transition-colors shadow-sm"
          >
      ...
      <div className="px-6 pt-4 pb-2 relative z-10">
        <div className="max-w-3xl mx-auto relative">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {(['SQL', 'JAVASCRIPT', 'PYTHON'] as ProcedureLanguage[]).map((language) => (
                        <button
                          ...
                          className={`px-2.5 py-1 rounded-md border text-xs font-medium ${procedureQuickConfig.language === language ? 'bg-blue-50 border-blue-300 text-blue-700' : 'bg-white border-border text-text-main hover:bg-slate-50'}`}
                        >
                          {language}
                        </button>
                      ))}
                    ...
                    <div className="flex flex-wrap gap-2">
                      {(['CALLER', 'OWNER'] as Array<'CALLER' | 'OWNER'>).map((mode) => (
                        <button
                          ...
                          className={`px-2.5 py-1 rounded-md border text-xs font-medium ${procedureQuickConfig.executeAs === mode ? 'bg-blue-50 border-blue-300 text-blue-700' : 'bg-white border-border text-text-main hover:bg-slate-50'}`}
                        >
                          {mode}
                        </button>
                      ))}
                  ...
                  <div className="mb-3">
                    ...
                    <div className="flex flex-wrap gap-2">
                      <button
                        ...
                        className={`px-2.5 py-1 rounded-md border text-xs font-medium ${procedureQuickConfig.includeLogging ? 'bg-blue-50 border-blue-300 text-blue-700' : 'bg-white border-border text-text-main hover:bg-slate-50'}`}
                      >
                        Yes
                      </button>
                      <button
                    ...
                    <div className="flex flex-wrap gap-2">
                      {(['TEXT', 'TABLE'] as Array<'TEXT' | 'TABLE'>).map((returnType) => (
                        <button
                          ...
                          className={`px-2.5 py-1 rounded-md border text-xs font-medium ${procedureQuickConfig.returnStyle === returnType ? 'bg-blue-50 border-blue-300 text-blue-700' : 'bg-white border-border text-text-main hover:bg-slate-50'}`}
                        >
                          {returnType}
                        </button>
                      ))}
                    ...
                    {guidedCurrentStep.options.map((option) => {
                      ...
                      return (
                        <button
                          ...
                          className={`w-full text-left px-3 py-2 rounded-lg border text-sm transition-colors ${selected ? 'bg-blue-50 border-blue-300 text-blue-800' : 'bg-white border-border text-text-main hover:bg-slate-50'}`}
                        >
                          {option}
                        </button>
                      );
          ...
          >
            ...
            <div className="flex items-center justify-between border-t border-border px-3 py-2 bg-white/80">
              <div className="flex items-center gap-2">
                <button
                  type="button"
                  title="Add context"
                  className="p-1.5 rounded-md border border-border text-text-muted hover:text-text-main hover:bg-slate-50 transition-colors"
                >
                ...
                >
                  {schemaOptions.map((schema) => (
                    <option key={schema} value={schema}>{schema === 'AUTO' ? 'Schema: Auto' : `Schema: ${schema}`}</option>
                  ))}
                </select>
                <button
                  type="button"
                  onClick={() => setThinkingEnabled(!thinkingEnabled)}
                  className={`flex items-center gap-1.5 px-2 py-1 border rounded-md text-xs font-medium transition-colors ${thinkingEnabled ? 'bg-blue-50 border-blue-200 text-blue-700' : 'bg-white border-border text-text-main hover:bg-slate-50'}`}
                  title="Thinking"
                >
              ...
              <div className="text-[11px] text-text-muted font-medium pr-1">
                {isGenerating && thinkingEnabled ? 'Thinking step-by-step...' : 'Ready'}
              </div>
            </div>
          </form>
