...
export function Button({
  ...
  return (
    <button
      ...
      onClick={handleClick}
      className={clsx(
        'group relative inline-flex items-center justify-center font-medium rounded-xl transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-offset-1 overflow-hidden select-none',
        {
          'bg-[var(--color-accent)] text-white hover:bg-[var(--color-accent-hover)] shadow-sm hover:shadow-md focus:ring-[var(--color-accent)]/40 active:scale-[0.97]':
            variant === 'primary',
          'bg-white text-[var(--color-text-main)] border border-[var(--color-border)] hover:bg-[var(--color-surface)] hover:border-[var(--color-border-strong)] hover:shadow-sm focus:ring-[var(--color-accent)]/20 active:scale-[0.97]':
            variant === 'secondary',
